...
const CheckListItem: React.FC<CheckItem & { index: number }> = ({ text, index }) => (
  <li 
    key={index} 
    className="flex items-start p-1 rounded-lg transition duration-200 hover:bg-gray-100/50 cursor-pointer"
  >
        ...
        <div className="relative overflow-hidden rounded-2xl shadow-xl bg-white transition duration-300 hover:shadow-2xl group border border-gray-100 transform hover:-translate-y-1">
            ...
            <div className="p-6">
                <div className="flex items-start mb-3">
                    <div className={`p-3 rounded-full bg-blue-50/70 mr-4 flex-shrink-0 ${block.color} transition duration-300 group-hover:bg-blue-100/90`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <ul className="space-y-2">
                {col1.map((benefit, index) => (
                  <CheckListItem key={index} text={benefit} index={index} />
                ))}
              </ul>
    ...
    const TabLink: React.FC<{ id: string, label: string }> = ({ id, label }) => {
        ...
        return (
            <button
                ...
                className="group relative w-full text-left py-3 px-4 transition duration-300 focus:outline-none hover:bg-gray-100/50 rounded-lg" // Added hover
            >
                {/* Vertical Accent Bar */}
                <div 
        ...
        <div className="py-16 sm:py-24 lg:py-32 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="lg:grid lg:grid-cols-12 lg:gap-12 mt-12">
                    ...
                    <div key={activeTabId} className="lg:col-span-8 mt-8 lg:mt-0 p-6 sm:p-8 bg-gray-50 rounded-2xl shadow-xl transition-all duration-300">
                            ...
                            <div>
                                <ul className="space-y-4">
                                    {activeTab.checkItems.map((item, index) => (
                                        <CheckListItem key={index} text={item.text} index={index} />
                                    ))}
                                </ul>
                            </div>
